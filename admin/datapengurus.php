...
<section class="pl-2 pt-5 " id="main-content">
  <div class="col-md-30 p-1 pt-2">
    <h3><i class="fas fa-user-alt mr-2"></i>DATA PENGURUS</h3><hr>
    ...
    <table class="table table-striped table-bordered border-dark shadow h-100">
      <thead>
        <tr>
          ...
          <th>ALAMAT</th>
          <th>JABATAN DIPENGURUSAN</th>
          <th>AKSI</th>
        </tr>
      </thead>
      <tbody>
        <div class="tambah">
          <a href="tambah_pengurus.php" class="btn btn-primary mb-3"><i class="far fa-plus"></i> Tambah</a>
        </div>
        <?php
        include "koneksi.php";
        $qry_pengurus=mysqli_query($koneksi, "select * from pengurus join jabatan on jabatan.id_jabatan=pengurus.id_jabatan");
        $no=0;
        while ($data_pengurus=mysqli_fetch_array($qry_pengurus)) {
          $no++;?>
          <tr>
            <td><?=$no?></td>
            <td><?=$data_pengurus['nama_pengurus']?></td>
            <td><?=$data_pengurus['tetala']?></td>
            <td><?=$data_pengurus['alamat']?></td>
            <td><?=$data_pengurus['nama_jabatan']?></td>

            <td><a href="edit_pengurus.php?id_pengurus=<?=$data_pengurus['id_pengurus']?>"class="btn btn-success"><i class="fal fa-edit fa-sm"></i></a> |
              <a href="delete_pengurus.php?id_pengurus=<?=$data_pengurus['id_pengurus']?>"onclick="return confirm('Apakah anda yakin menghapus data ini?')" class="btn btn-danger">

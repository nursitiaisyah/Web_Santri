...
<section class="pl-2 pt-5 " id="main-content">
  <div class="col-md-30 p-1 pt-2">
    <h3><i class="fal fa-chart-line "></i>  INFORMASI</h3><hr>
    ...
    <table class="table table-striped table-bordered border-dark shadow h-100">
      ...
      <tbody>
        <div class="tambah">
          <a href="tambah_informasi.php" class="btn btn-primary mb-3"><i class="far fa-plus"></i> Tambah</a>
        </div>
        <?php
        include "koneksi.php";
        $qry_informasi=mysqli_query($koneksi, "select * from informasi");
        $no=0;
        while ($data_informasi=mysqli_fetch_array($qry_informasi)) {
          $no++;?>
          <tr>
            <td><?=$no?></td>
            <td><?=$data_informasi['tgl_publikasi']?></td>
            <td><?=$data_informasi['pengumuman']?></td>

            <td><a href="edit_informasi.php?id_informasi=<?=$data_informasi['id_informasi']?>"class="btn btn-success"><i class="fal fa-edit fa-sm"></i></a> |
              <a href="delete_informasi.php?id_informasi=<?=$data_informasi['id_informasi']?>"onclick="return confirm('Apakah anda yakin menghapus data ini?')" class="btn btn-danger">

...
<section class="pl-2 pt-5 " id="main-content">
  <div class="col-md-30 p-1 pt-2">
    <h3><i class="fal fa-chart-line "></i>  JADWAL KEGIATAN</h3><hr>
    ...
    <table class="table table-striped table-bordered border-dark shadow h-100">
      ...
      <tbody>
        <div class="tambah">
          <a href="tambah_jadwalkegiatan.php" class="btn btn-primary mb-3"><i class="far fa-plus"></i> Tambah</a>
        </div>
        <?php
        include "koneksi.php";
        $qry_jadwalkegiatan=mysqli_query($koneksi, "select * from jadwalkegiatan");
        $no=0;
        while ($data_jadwalkegiatan=mysqli_fetch_array($qry_jadwalkegiatan)) {
          $no++;?>
          <tr>
            <td><?=$no?></td>
            <td><?=$data_jadwalkegiatan['jam']?></td>
            <td><?=$data_jadwalkegiatan['nama_kegiatan']?></td>

            <td><a href="edit_jadwalkegiatan.php?id_jadwal=<?=$data_jadwalkegiatan['id_jadwal']?>"class="btn btn-success"><i class="fal fa-edit fa-sm"></i></a> |
              <a href="delete_kegiatan.php?id_jadwal=<?=$data_jadwalkegiatan['id_jadwal']?>"onclick="return confirm('Apakah anda yakin menghapus data ini?')" class="btn btn-danger">
